...
| 기능         | 상태 | 적용                                            | 남은 작업                  |
| ------------ | ---- | ----------------------------------------------- | -------------------------- |
| Base URL     | 🟡   | 로컬 `http://localhost:8080`                    | 개발/AWS/운영 주소 분리    |
...
| `Timeline`     | ✅   | 목록에서 FIRE/ICE·본문·시각 표시                   | 만료 타이머 실서버 확인    |
| `MemoEditor`   | 🟡   | 생성/수정, KST DateTime                            | 이미지 선택·업로드 UI      |
...
| 페이지              | 상태 | 연결된 내용                            | 다음 작업                            |
| ------------------- | ---- | -------------------------------------- | ------------------------------------ |
| `TeamSpace/Home`    | ✅   | 내 팀 목록과 memberCount, N+1 제거     | 실서버 확인                          |
| `TeamCreate`        | ✅   | 팀 생성, 친구 검색·추천·선택·멤버 추가 | 부분 실패 응답 실서버 확인           |
| `ProjectDetail`     | 🟡   | 공지 CRUD, 할 일 CRUD·체크, 월별 일정  | 일정 상세·수정                       |
...
| `EditSchedule`      | 🔴   | 기존 더미 화면                         | 일정 상세 API 또는 목록 content 필요 |
| `Files`             | 🟡   | 목록·검색·다운로드·이름 변경·삭제      | 파일 선택기, 업로드, 이동/공유 계약  |
| `FriendAdd`         | ✅   | 검색·추천·친구 요청, 받은 요청 수      | 중복 요청 UX                         |
| `FriendRequestList` | ✅   | 받은/보낸 목록·수락·거절·취소          | 두 계정 E2E                          |
...
| `EditProfile`         | 🟡   | 닉네임 수정                                       | 이미지 선택 → PROFILES 업로드         |
...
| `VerifyCode`          | ✅   | 2FA 코드 검증, access/refresh 교체                | 재발송 계약 확인                      |
